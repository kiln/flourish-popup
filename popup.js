...
function textToHtml(text$$1) {
	return text$$1.replace(/[&<>]/g, function (s) {
		return ({ "&": "&amp;", "<": "&lt;", ">": "&gt;" })[s];
	});
}
...
Popup.prototype.on = function Popup_on(event$$1, handler) {
	if (!(event$$1 in this.handlers)) throw "Kiln.popup.on: No such event: " + event$$1;
	this.handlers[event$$1].push(handler);
	return this;
};

// Fire event
Popup.prototype.fire = function Popup_fire(event$$1, d) {
	if (!(event$$1 in this.handlers)) throw "Kiln.popup.fire: No such event: " + event$$1;
...
function positionBox(dir, s, path$$1, w, h, x, y, clientX, clientY, cb) {
	var lt = locateTriangle[dir](w, h, clientX, clientY, cb);
	s.left = (x - BORDER - lt.pos[0]) + "px";
	s.top = (y - BORDER - lt.pos[1]) + "px";
	path$$1.setAttribute("d", "M0,0L" + lt.shape.join(",") + "Z");
	path$$1.setAttribute("transform", "translate(" + (lt.pos[0] + BORDER) + "," + (lt.pos[1] + BORDER) + ")");
}
...
Popup.prototype.draw = function Kiln_popup_draw() {
	...
	positionBox(dir, s, path$$1, w, h, x, y, clientX, clientY, cb);
	return this;
};
